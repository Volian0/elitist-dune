...
int main(int, char*[])
{
 ...
 // create SDF texture
    ...
    const auto draw_quad = [&](const glm::uvec2& t_texture_position, const glm::uvec2& t_texture_size,
        ...
        const auto draw_quad_sdf = [&](const glm::uvec2& t_texture_position, const glm::uvec2& t_texture_size, unsigned t_atlas_size,
                               const glm::vec2& t_position, const glm::vec2& t_size, const glm::mat4& t_matrix,
                               const glm::vec4& t_top_tint = glm::vec4{1.0F},
    ...
    const auto draw_text_ascii = [&](const Font& t_font, const glm::vec2& t_position, std::string_view t_text, const glm::mat4& t_matrix,
                               float t_scale = 1.0F, const glm::vec4& t_tint_top = glm::vec4{1.0F}, const glm::vec4& t_tint_bottom = glm::vec4{1.0F}, float t_thickness = 0.5F) {
        ...
        for (char c : t_text) //TODO: at() -> [], const auto& glyph instead of at
        {
            ...
            glm::vec2 padded_render_size = unpadded_render_size * (glm::vec2(glyph.atlas_size-t_font.padding*2)/glm::vec2(glyph.atlas_size));
            ...
            draw_quad_sdf(glyph.atlas_position, glyph.atlas_size, t_font.atlas_size,
            render_position, unpadded_render_size * (unpadded_render_size / padded_render_size), t_matrix, t_tint_top, t_tint_bottom, t_thickness);
        }
    };
    ...
    const auto tp_start = tp_old;
        ...
        const auto draw_single_tile = [&](float t_column_position, float t_position_y, float t_length, std::optional<float> t_clear_progress = std::nullopt){
            ...
            if (!t_clear_progress)
                {
                    draw_quad({460, 511}, {0, 0}, {t_column_position, t_position_y},
                              {1.0F, t_length}, matrix_level, BLACK, BLACK);
                }
                else
                {
                    float actual_border = (1.0F - glm::min(*t_clear_progress / animation_time , 1.0F)) * border;
                    draw_quad({460, 511}, {0, 0}, {t_column_position, t_position_y},
                        {1.0F, t_length}, matrix_level, BLACK_CLEAR, BLACK_CLEAR);
                    if (actual_border > 0.0F) //TODO: BORDER -> ACTUAL BORDER
                    {
                        ...
                        draw_quad({460, 511}, {0, 0}, {t_column_position, t_position_y + t_length - actual_border * aspect_ratio},
                              {1.0F, actual_border * aspect_ratio}, matrix_level, BLACK, BLACK);
                    }
                }
        };
    while (true)
    {
        ...
        for (SDL_Event event; SDL_PollEvent(&event);)
        {
            ...
            {
                if (event.tfinger.y > 0.75F && level.idle && level.revives_used == 0 && level.tempo <= 20.0F)
                {
                    level.tempo += 0.2F;
                }
                else {level.touch_down({event.tfinger.x, event.tfinger.y}, event.tfinger.fingerID, tp_new, soundfont);}
        ...
        {
            float size = 0.02F;
            draw_quad({449, 511}, {9, 0}, {float(column_i) - size * 0.5F, 0.0F},
                        {size, 4.0F}, matrix_level);
        }
        for (const auto& tile : level.tiles)
        {
            ...
            if (type == TileInfo::Type::LONG)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        draw_quad({460, 511}, {0, 0},
                                  {column_position, -(tile.position - level.position - 4.0F) - delta_minus_clearer}, {1.0F, delta_minus_clearer},
                                  matrix_level, CLEAR_BLUE, TOP_BLUE);
                    }
                }
            ...
            {
                draw_single_tile(column_position, -(tile.position - level.position - 4.0F) - length, length, tile.cleared_column != Column::NONE ? std::optional<float>(time_left) : std::nullopt);
                /*if (tile.cleared_column == Column::NONE)
                {
                    draw_quad({460, 511}, {0, 0}, {column_position, -(tile.position - level.position - 4.0F) - length},
            ...
            else if (type == TileInfo::Type::DOUBLE)
            {
                ...
                {
                    draw_single_tile(0.0F, -(tile.position - level.position - 4.0F) - length, length, (unsigned(tile.cleared_column) & unsigned(Column::FAR_LEFT)) ? std::optional<float>(time_left) : std::nullopt);
                    draw_single_tile(2.0F, -(tile.position - level.position - 4.0F) - length, length, (unsigned(tile.cleared_column) & unsigned(Column::CENTER_RIGHT)) ? std::optional<float>(time_right) : std::nullopt);
                }
                else
                {
                    draw_single_tile(1.0F, -(tile.position - level.position - 4.0F) - length, length, (unsigned(tile.cleared_column) & unsigned(Column::CENTER_LEFT)) ? std::optional<float>(time_left) : std::nullopt);
                    draw_single_tile(3.0F, -(tile.position - level.position - 4.0F) - length, length, (unsigned(tile.cleared_column) & unsigned(Column::FAR_RIGHT)) ? std::optional<float>(time_right) : std::nullopt);
                }
            }
        }
        ...
        draw_text_debug({32, 32}, "FPS: " + std::to_string(glm::floor(1.0F / delta_time)), matrix_test, 2.0F,
                  glm::vec4{1.0, 0.047, 0.047, 1.0});*/
        /*draw_text_ascii(font, {32,64}, "Score: " + std::to_string(level.score), matrix_test, 0.001F, glm::vec4(0.0F, 0.0F, 0.0F, 1.0F), 0.9);
        draw_text_ascii(font, {32,64}, "Score: " + std::to_string(level.score), matrix_test, 0.001F, glm::vec4(1.0F, 0.1F, 0.1F, 1.0F));
        draw_text_ascii(font, {32,64+64}, "TPS: " + std::to_string(level.tempo), matrix_test, 0.001F, glm::vec4(0.0F, 0.0F, 0.0F, 1.0F), 0.9);
        draw_text_ascii(font, {32,64+64}, "TPS: " + std::to_string(level.tempo), matrix_test, 0.001F, glm::vec4(1.0F, 0.1F, 0.1F, 1.0F));*/
        
        std::string string_score{std::to_string(level.score_display)};
        float additional_score_size = (0.05F - glm::min(std::chrono::duration<float>(tp_new - level.time_last_score_update).count(), 0.05F)) * 0.00001F + 0.000003F;
        ...
        draw_text_ascii(font, {0.5-string_score_length*0.5F,0.125 / aspect_ratio}, string_score, matrix_aspect, additional_score_size, glm::vec4(0.01F, 0.0F, 0.0F, 1.0F), glm::vec4(0.0F, 0.0F, 0.0F, 1.0F), 0.9);
        draw_text_ascii(font, {0.5-string_tps_length*0.5F,0.125 / aspect_ratio + 0.09}, string_tps, matrix_aspect, 0.0000015F, glm::vec4(0.01F, 0.0F, 0.0F, 1.0F), glm::vec4(0.0F, 0.0F, 0.0F, 1.0F), 0.9);
        draw_text_ascii(font, {0.5-string_score_length*0.5F,0.125 / aspect_ratio}, string_score, matrix_aspect, additional_score_size, glm::vec4(1.0F, 0.1F, 0.1F, 1.0F), glm::vec4(1.0F, 0.2F, 0.2F, 1.0F));
        draw_text_ascii(font, {0.5-string_tps_length*0.5F,0.125 / aspect_ratio + 0.09}, string_tps, matrix_aspect, 0.0000015F, glm::vec4(1.0F, 0.9F, 0.9F, 1.0F), glm::vec4(1.0F, 1.0F, 1.0F, 1.0F));
        ...
        {
            const float tile_length = float(level.song_info.get_tiles()[active_tile->id].get_unit_length())/float(level.song_info.get_length_units_per_single_tile());
            bool is_double_tile = level.song_info.get_tiles()[active_tile->id].get_type() == TileInfo::Type::DOUBLE;
            float column_position = is_double_tile ? (active_tile->column == Column::DT_LEFT ? 0.0 : 1.0) : column_to_position(active_tile->column);
            glm::vec4 position_a = matrix_level * glm::vec4(column_position, -active_tile->position - tile_length + level.position + 4.0F, 0.0, 1.0);
            glm::vec4 position_c = matrix_level * glm::vec4(column_position + 1.0, -active_tile->position + level.position + 4.0F, 0.0, 1.0);
            buffer_vbo_helper = {
                position_a.x, position_a.y, 0.0, 0.0,
                position_c.x, position_a.y, 1.0, 0.0,
                position_c.x, position_c.y, 1.0, 1.0,
                position_a.x, position_c.y, 0.0, 1.0,
                position_a.x + 1.0F, position_a.y, 0.0, 0.0,
                position_c.x + 1.0F, position_a.y, 1.0, 0.0,
                position_c.x + 1.0F, position_c.y, 1.0, 1.0,
                position_a.x + 1.0F, position_c.y, 0.0, 1.0,
            };
            glBindVertexArray(vao_helper);
            glBindBuffer(GL_ARRAY_BUFFER, vbo_helper);
            glUseProgram(program_helper);
            static float last_tile_length = 1.0F;
            float alpha = std::min(glm::pow(std::chrono::duration<float>(tp_new - level.tile_full_time_clicked).count() * level.tempo, 2.0F), 1.0F);
            ...
            glDrawElements(GL_TRIANGLES, (is_double_tile && active_tile->cleared_column == Column::NONE) ? 12 : 6, GL_UNSIGNED_BYTE, reinterpret_cast<void*>((is_double_tile && (unsigned(active_tile->cleared_column) & 0b1100)) ? 6 : 0));
        }
        //draw SDF
        //glDisable(GL_BLEND);
        glBindVertexArray(vao_sdf);
